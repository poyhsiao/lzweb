...
class dhcp_lan(object):

	def __init__(self):
		self.msg = []
		self.caption = "IPv4 DHCP (LAN) Setting"
	...
	def dhcpHandler(self, **kwargs):
		...
		ret = xte_dhcp_lan.set(dhcp_jcfg, str(cherrypy.request.login))
		tmp = controller.lookup.get_template("dhcp.mako")
		if ret[0]: # save successfully
			self.addMsg(_("Settings are applied for page Service -> DHCP (LAN)"))
